# A9G port of DFRobot_BMP388 micropython library
# for BMP388 temperature and barometric pressure sensor via i2c
# emmanuel@interspecifics.cc   MMXXIII
# -------------------------------------------------------------
...
class DFRobot_BMP388:
  def __init__(self):
    ...
    self.par_p11 = 0
    self.addr = 119
    print("getting the regs")
    chip_id = self.bmp3_get_regs(0x00, 1)
    print("BMP388 chip ID:", end='')
    print(hex(int.from_bytes(chip_id, 'little')))
    #if (chip_id != 0x50):
    #  print("chip id error!")
    #  sys.exit()
    self.get_calib_data()
...
# this is the I2C subclass of BMP388
class DFRobot_BMP388_I2C(DFRobot_BMP388):
  def __init__(self,i2c):
    self.i2c = i2c
    super(DFRobot_BMP388_I2C,self).__init__()

  def bmp3_get_regs(self,reg,lenn):
    #rslt = self.i2c.readfrom_mem(self.addr,reg,len)
    rslt = self.i2c.mem_receive(2, self.addr, reg, 1, lenn, 10)
    return rslt

  def bmp3_set_regs(self,reg,data):
    #self.i2c.writeto_mem(self.addr,reg,data)
    self.i2c.mem_transmit(2, self.addr, reg, 1, data, 10)